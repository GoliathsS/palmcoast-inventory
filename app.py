...
def scan_action():
    ...
    if result:
        ...
        if direction == 'out':
            ...
            # 2. Get vehicle assigned to the technician by name
            cur.execute("""
                SELECT v.vehicle_id
                FROM vehicles v
            ...
            if vehicle_row:
                ...
                # 3. Update vehicle_inventory with quantity and timestamps
                cur.execute("""
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, NOW(), %s, %s)
        """, (product_id, direction, technician, unit_cost if direction == 'out' else round(unit_cost * units_per_item, 2)))
